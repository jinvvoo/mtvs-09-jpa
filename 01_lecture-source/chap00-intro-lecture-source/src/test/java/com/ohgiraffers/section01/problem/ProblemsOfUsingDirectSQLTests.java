...
public class ProblemsOfUsingDirectSQLTests {
    ...
    * */
    ...
    @Test
    void testChangSelectColumns() throws SQLException {
        ...
        while (rset.next()) {
            ...
            menu.setMenuName(rset.getString("MENU_NAME"));
            // price 추가해줘 요청들어옴
//            menu.setMenuPrice(rset.getInt("MENU_PRICE")); // 에러남 given에 SELECT도 수정해야함
    ...
    *   하지만 객체지향의 상속은 슈퍼타입의 속성을 ㄱㅇ유해서 사용하기 때문에 여기서 패러다임 불일치 현상이 발생하게 된다.
    * */

    /* 설명.
    *   법인 회원과 일반 회원이라는 두 가지 회원을 추상화하여 회원이라는 슈퍼타입 엔터티를 도출하고
    *   서브타입 엔터티로 법인회원과 일반회원의 존재하는 상황인 경우
    ...
    *       private String menuName;
    *       private int menuPrice;
    *       private Category categoryCode;          //여기
    *       private String orderableStatus;
    *   }
    *
    * 설명. 불일치 문제 해결할 때 이런식으로
    *  Menu menu = new Menu();
    *  Category category = new Category();
    *  menu.setCategory(category);          // 메뉴와 카테고리의 관계설정을 개발자가 직접.
    ...
    *   menu.getCategory().getCategoryName();       // NPE 발생하지 않음을 보장 (nullpointexception)
    * */
