...
public class EntityLifeCycleTests {
    ...
    /* 필기.
     *   영속성 컨텍스트(persistence context)       // 컬렉션이라고 생각해도 됨.
     ...
     *   준영속(detached) : 영속성 컨텍스트에 저장되었다가 분리된 상태 (db에 삭제하지 않음)
     *   삭제(removed) : 준영속과 유사해보이지만, 아에 삭제가 된 상태 (db에 삭제함)
     *
     * */
    ...
    void testmanagerdNewEntity(String menuName, int menuPrice, int categoryCode, String orderableStatus) {
    ...
    void testDetachEntity(int menuCode, int menuPrice) {
        ...
        //then
        Assertions.assertNotEquals(menuPrice, manager.find(Menu.class, menuCode));
        transaction.rollback();
    }
...
//        manager.persist(foundMenu);       // 오류.
    ...
    void testClearPersistenceContext(int menuCode) {
        ...
        //given
        EntityManager manager = EntityManagerGenerator.getManagerInstance();
        Menu fonudMenu = manager.find(Menu.class, menuCode);
    ...
    @ParameterizedTest
    @ValueSource(ints = {1})
    void testClosePersistencContext(int menuCode) {
