...
public class EntityManagerGeneratorTests {
    ...
    *   쓰레드 세이프이기 때문에 여러 쓰레드가 동시에 접근해도 안전하므로
    *   서로 다른 쓰레드간 공유해서 재사용한다.
    *   쓰레드 세이프한 기능을 요청 스코프마다 생성하기에는 비용(시간, 메모리)부담이 크기 때문에
    ...
    * */
    ...
    void testGeneratorEntityManagerFactory() {
        ...
        //then
        Assertions.assertNotNull(factory);  // 생성한 factory가 정상?
    ...
    *    엔티티 매니저는 엔티티를 저장하는 메모리상의 데이터베이스를 관리하는 인스턴트이다.
